...
- [权限及用户系统](#权限及用户系统)
- [通知及社区互动](#通知及社区互动)
- [运营工具 (Admin Tools)](#运营工具-admin-tools)
- [安全性架构](#安全性架构)
- [SEO及分析工具](#seo及分析工具)
- [多语言支持](#多语言支持)
- [插件系统](#插件系统)
...
### 🔔 实时通知系统
- **动态提醒**: 当您的帖子收到评论或点赞时，系统会立即发送通知。
- **系统通知**: 实时接收等级提升、积分变动或管理员发布的公告。
- **用户界面**: 导航栏显示未读计数，支持在通知中心进行统一标记和管理。
...
### 💾 备份与恢复
- **一键备份**: 实时将当前数据库状态保存为 SQL 文件供下载。
- **管理与清理**: 轻松查看备份记录、下载至本地或删除过期备份文件。
- **数据恢复**: 支持通过上传备份文件或从列表中选择记录，快速回滚至特定时间点。

### 📋 活动日志与审计
- **管理员日志**: 详细记录管理员的所有操作（如修改配置、禁言用户等）。
- **登录记录审计**: 记录所有成功或失败的登录尝试，包含 IP 地址及浏览器环境。
- **检索与过滤**: 支持按日期、用户 ID 或操作类型快速检索关键日志。

### 📁 资源与资产管理
- **文件管理器**: 提供已上传文件总量、存储空间占用情况及文件类型统计。
- **孤立文件清理**: 自动扫描并清除数据库中不存在的冗余附件，释放服务器空间。

### 💰 积分及用户等级维护
- **手动积分调整**: 管理员可直接对特定用户的积分进行增加或扣除操作。
- **自动升级设置**: 支持根据用户累积的积分，自动提升其会员等级。

### 🚧 站点运维管理
- **维护模式**: 在系统更新时锁定站点，仅允许管理员访问，并向访客显示自定义维护页面。
- **违禁词过滤**: 设置敏感词库，系统会自动将帖子或评论中的违禁词替换为 (`***`)。

---

## 安全性架构

### 🛡️ 基础安全防御 (Fundamental Security)
- **CSRF 防护**: 为所有表单请求分配并验证唯一令牌。
- **XSS 防御**: 通过高级过滤机制，杜绝恶意脚本注入。
- **SQL 注入防御**: 全面采用 PDO 预处理语句处理数据库查询。

### 🔐 认证及访问管控
- **二步验证 (2FA)**: 支持邮件或身份验证器 (OTP) 提供的二次安全认证。
- **暴力破解防护**: 登录失败次数过多时，系统将自动锁定该 IP。
- **管理员 IP 白名单**: 仅允许来自指定 IP 的请求进入管理后台，二级加固账号安全。

### 🌐 系统级安全保护
- **强制安全标头**: 自动应用 HSTS、X-Frame-Options、Nosniff 等 Web 安全标头。
- **IP 封禁系统**: 永久或限时隔离恶意尝试访问站点的 IP 地址。
- **随机化文件名**: 自动更改上传的文件名
- **执行防护**: 阻止上传目录中的脚本执行
