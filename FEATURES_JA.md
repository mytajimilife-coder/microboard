...
- [コア機能](#コア機能)
- [掲示板およびページ管理](#掲示板およびページ管理)
- [権限および会員システム](#権限および会員システム)
- [通知およびコミュニティ](#通知およびコミュニティ)
- [運用ツール (Admin Tools)](#運用ツール-admin-tools)
- [セキュリティアーキテクチャ](#セキュリティアーキテクチャ)
- [SEOおよび分析ツール](#seoおよび分析ツール)
- [多言語サポート](#多言語サポート)
- [プラグインシステム](#プラグインシステム)
- [管理者機能](#管理者機能)
- [セキュリティ機能](#セキュリティ機能)
...
MicroBoard 内で静的なページ（例：会社概要、利用規約など）を直接作成・管理できます。

### 主な機能
- **ページ作成**: 管理画面でタイトル、内容、URLスラッグを設定
- **権限設定**: ページごとに閲覧権限レベルを設定（ゲスト、会員専用など）
- **変数使用**: ページ内容内で `{{変数名}}` の形式で動的データを挿入可能
- **URLアクセス**: `page.php?id=スラッグ` の形式でアクセス

### カスタム変数 (Custom Variables)
- 管理画面の設定ページで独自の変数を定義できます。
- **例**: `{{twitter_url}}` 変数を作成し、値をリンクに設定すると、投稿やページ本文で `{{twitter_url}}` と入力した際に自動的にそのリンクに置換されます。
- **デフォルト変数**:
  - `{{site_title}}`: サイトタイトル
  - `{{copyright}}`: 著作権テキスト
  - `{{username}}`: 現在のユーザーID
  - `{{nickname}}`: 現在のユーザーニックネーム
  - `{{year}}`: 現在の西暦年
...
### 🔔 リアルタイム通知システム
- **活動通知**: 自分の投稿にコメントや「いいね」がつくとすぐに通知。
- **システム通知**: レベルアップ、ポイント変動、管理者からのお知らせをリアルタイムで受信。
- **UI連携**: ヘッダーに未読件数を表示し、通知センターで統合管理が可能。

---

## 運用ツール (Admin Tools)

効率的なサイト運営のための強力なツール群を提供します。

### 💾 バックアップおよび復元
- **ワンクリックバックアップ**: 現在のDB状態をSQLファイルとして即座に保存。
- **管理およびダウンロード**: バックアップ履歴の確認、ダウンロード、不要なファイルの削除が容易。
- **復元機能**: アップロードまたはリスト選択により、過去の時点にシステムを復旧。

### 📋 アクティビティログと監査
- **管理者ログ**: 設定変更、掲示板作成、会員規制など、管理者の全操作を記録。
- **ログイン監査**: 成功・失敗を問わず、IPやブラウザ情報を含む全ログイン試行を記録。
- **検索とフィルタリング**: 日付、ユーザー、アクション別にログを高速検索。

### 📁 リソースおよび資産管理
- **ファイルマネージャー**: アップロードされたファイル総量や種類別の統計データを提供。
- **孤立ファイルの自動整理**: DBに存在しない不要なファイルをスキャンして削除し、サーバー容量を最適化。

### 💰 ポイントおよび会員ランク
- **ポイント手動調整**: 管理者が特定のユーザーに対してポイントを付与・没収可能。
- **自動レベルアップ**: 設定された累積ポイントに到達すると自動的に会員ランクを昇格。

### 🚧 システム運営設定
- **メンテナンスモード**: アップデート時に一般ユーザーのアクセスを制限し、告知ページを表示。
- **NGワードフィルター**: 登録された不適切なワードを自動的に伏せ字(`***`)で処理。

---

## セキュリティアーキテクチャ

### 🛡️ 基礎セキュリティ (Fundamental Security)
- **CSRF保護**: すべてのフォームリクエストに対して固有のトークンを検証。
- **XSS防止**: 悪意のあるスクリプトの実行を高度なフィルタリングで遮断。
- **SQL Injection防止**: PDO Prepared StatementsによりDBクエリを安全に処理。

### 🔐 認証およびアクセス制御
- **2要素認証 (2FA)**: メールまたは認証アプリ(OTP)による強力なログイン保護。
- **Brute Force防御**: ログイン失敗が繰り返された場合に、該当IPを自動的にロック。
- **管理者IPホワイトリスト**: 許可されたIPからのみ管理者ダッシュボードへのアクセスを許可。

### 🌐 システムレベルの保護
- **セキュリティヘッダー**: HSTS、X-Frame-Options、Nosniffなどの各種ヘッダーを強制適用。
- **IP拒否管理**: 不正アクセスを繰り返すIPを永久または一時的に遮断。
